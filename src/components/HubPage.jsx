...
// IMPORTANTE: Añadimos query, orderBy, limit, getDocs, updateDoc
...
const HubPage = ({
    ...
    // --- NUEVO: Estados para las Notificaciones/Anuncios ---
    ...
    // --- NUEVO: Efecto para buscar el último anuncio ---
    useEffect(() => {
        const fetchUltimoAnuncio = async () => {
            try {
                ...
                if (!snap.empty) {
                    ...
                    // Si el usuario no tiene la variable o no coincide con el anuncio actual, hay notificación
                    if (usuario?.ultimoAnuncioVisto !== anuncioId) {
        ...
        // Solo buscamos si el usuario ya está cargado
        if (usuario?.uid) {
            fetchUltimoAnuncio();
        }
    }, [usuario]);

    // --- NUEVO: Función para abrir el anuncio y marcarlo como leído ---
    const handleAbrirAnuncio = async (e) => {
        e.stopPropagation(); // Evita que se abra 'Mi Cuenta' al hacer clic en la campanita
        setShowAnuncio(true);
        setHayNotificacion(false); // Apagamos la campanita visualmente al instante
    ...
    return (
        <div style={contenedorEstilos}>
            ...
            {/* BOTONES DEL HUB (Ocultos para ahorrar espacio en este ejemplo, pero MANTENLOS IGUAL) */}
            ...
            {/* --- MODAL DE ACTUALIZACIONES (NUEVO) --- */}
            ...
            {/* --- MODAL DE SOPORTE (Original) --- */}
            {/* Mantén tu código original aquí */}
            
            {/* --- MODAL DE TUTORIALES (Original) --- */}
            {/* Mantén tu código original aquí */}
