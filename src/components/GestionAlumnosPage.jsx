...
const GestionAlumnosPage = ({ onBack, styles, usuario }) => {
    ...
    const handleFotoChange = (e) => {
        ...
        if (!file) return;
        if (file.size > 1048487) return notify("La foto es muy pesada. Máximo 1MB.");
    ...
    const handleVincularAcademia = async () => {
        if (!codigoVinculacion.trim()) return notify("Pega un código válido.");
        try {
            await updateDoc(doc(db, "usuarios", usuario.uid), { academiaId: codigoVinculacion.trim() });
            notify("Vinculación exitosa. Reiniciando...");
            window.location.reload();
        } catch (e) { notify("Error al vincular."); }
    };
    ...
    const handleGuardarAlumno = async () => {
        if (!nuevo.nombre || !nuevo.fechaPago) return notify("Nombre y fecha de pago requeridos.");
    ...
    const handleRegistrarPago = async (alumno) => {
        ...
        try {
            ...
            notify("Error al actualizar el pago.");
        }
    };
    // --- UI HELPERS: CHIPS ---
                                            ...
                                            onClick={async () => {
                                                ...
                                                if (result.isConfirmed) {
                                                    try {
                                                        ...
                                                        // Notificación rápida tipo "Toast" para confirmar el éxito
                                                        Swal.fire({
                                                            toast: true,
                                                            position: 'top-end',
                                                            icon: 'success',
                                                            title: alumno.activo ? 'Enviado al Vault' : 'Guerrero reactivado',
                                                        ...
                                                        Swal.fire({
                                                            text: "No se pudo actualizar el estado. Revisa tu conexión.",
                                                            icon: 'error',
                                                            background: '#0a0a0a',
                                                            color: '#fff',
                                                            confirmButtonColor: '#d4af37'
                    ...
                    <div style={{ ...styles.card, width: '100%', maxWidth: '500px', maxHeight: '95vh', overflowY: 'auto', padding: '30px', border: '1px solid #d4af37' }}>
                        ...
                        <div style={{ marginBottom: '25px', borderTop: '1px solid #222', paddingTop: '20px' }}>
                            ...
                            <div style={{ backgroundColor: '#111', padding: '15px', borderRadius: '10px', border: '1px solid #d4af3744', marginBottom: '15px' }}>
                                ...
                                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                    ...
                                    <button onClick={() => { navigator.clipboard.writeText(usuario.uid); notify("Código copiado al portapapeles."); }} style={{ ...styles.btnGold, width: 'auto', padding: '10px' }}>📋</button>
                                </div>
                            </div>
                        ...
                        <div style={{ display: 'flex', gap: '15px', marginTop: '10px' }}>
                            <button onClick={async () => {
                                ...
                                notify("¡Configuración de Sede guardada y sincronizada! OSS.");
